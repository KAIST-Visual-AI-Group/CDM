...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        return_last_hidden_only: bool = False,
    ) -> LLaDAOutput:
        """
        ...
        collect_intermediate_hidden = output_hidden_states and not return_last_hidden_only
        collect_final_hidden = output_hidden_states or return_last_hidden_only
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                if collect_intermediate_hidden:
                    # add hidden states
        ...
        return LLaDAOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if (collect_intermediate_hidden or collect_final_hidden) else None)  # type: ignore[arg-type]
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model.forward(
            ...
            return_last_hidden_only=return_last_hidden_only,
        )
